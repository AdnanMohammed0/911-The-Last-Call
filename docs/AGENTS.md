# docs/AGENTS.md — تعليمات الذكاء الاصطناعي لمشروع 911: The Last Call
...
2. لا تعدّل ملف تقدم عضو آخر (`docs/data/progress/<id>.json`). كل عضو يحدّث ملفه فقط، وبهذي الطريقة ما يصير تعارض (merge conflict).
3. لا تغيّر `docs/data/tasks.json` يدوياً. المصدر هو `docs/design/PROJECT_ROADMAP.md`، وبعد تعديله شغّل `python docs/scripts/build_tasks.py`.
4. قبل الـ push: `python docs/scripts/validate_progress.py` ثم `git pull --rebase`.
...
```bash
python docs/scripts/whoami.py
```

- يطابق `git config user.email` / `user.name` / حساب `gh` مع `docs/data/team.json`.
...
| :--- | :--- | :--- | :--- | :--- |
| `adnan` | Adnan | AdnanMohammed0 | [`docs/prompts/adnan.md`](prompts/adnan.md) | `docs/data/progress/adnan.json` |
| `ali` | Ali Imad | Ali-Imad-Dev | [`docs/prompts/ali.md`](prompts/ali.md) | `docs/data/progress/ali.json` |
| `mohamed` | Mohamed | MohamedFSD | [`docs/prompts/mohamed.md`](prompts/mohamed.md) | `docs/data/progress/mohamed.json` |

## 2. حمّل سياق العضو (Load context)

اقرأ بالترتيب:

1. `docs/prompts/<id>.md`: دور العضو، مهامه، وقواعده الخاصة.
2. `docs/data/progress/<id>.json`: وين وصل.
3. `docs/data/tasks.json`: فلتر المهام على `"owner": "<id>"`.
4. الوثائق حسب الحاجة: `docs/design/ARCHITECTURE.md` و`docs/design/GAMEPLAY_MECHANICS.md` و`docs/design/PROJECT_ROADMAP.md` و`docs/design/game_design_document_911_The_Last_call.md`.
...
1. حدّث `docs/data/progress/<id>.json`:
...
```bash
python docs/scripts/validate_progress.py
git add -A
...
| الطبقة | شنو تسوي |
| :--- | :--- |
| **هذا الملف** (`docs/AGENTS.md`، يُحمَّل عن طريق `.claude/CLAUDE.md` و `.cursor/rules/`) | القاعدة الأساسية لأي AI |
| **Claude Code Stop hook** (`.claude/settings.json` → `docs/scripts/hooks/stop_check.py`) | إذا حاولت تنهي الجلسة وأكو تغييرات مو مرفوعة، يمنعك ويطلب تسجّل التقدم وتسوي push |
| **GitHub Action** (`.github/workflows/auto-progress.yml`) | أي push ما حدّث `docs/data/progress/` يتسجّل تلقائياً بملف صاحبه، والحالة تنقرأ من رسالة الـ commit (`done P1-04`، `test P1-04`، `blocked P1-04`) |
...
- **البديل** (بدون gh): أضف عنصر بـ `docs/data/ideas.json` بـ id جديد متسلسل (`I-007`…)، و`author` = id العضو، و`status` = `new`.

## 5. أسلوب الكود

- Godot 4.x + GDScript مع static typing كامل.
- اتبع `docs/design/ARCHITECTURE.md` §10 (Coding Standards) وقواعد الشبكة §4.3 (كل RPC من نوع `any_peer` يتحقق من المرسل).
